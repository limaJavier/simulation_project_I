...
class ImpatienceEvent(Event):
    def __init__(self, type: str, time: int, customer : int = None) -> None:
        super().__init__(type, time)
        self.customer = customer

class CarWash(Simulation):
    def __init__(self, interarrival_distro, service_distro, impatience_distro,maximum_queue_length = 9, duration = 36000) -> None:
        super().__init__()
        # Simulation times distributions
        self._interarrival_distro = interarrival_distro
        self._service_distro = service_distro
        self._impatience_distro = impatience_distro
    ...
    def _simulate_day(self, initial_state: tuple[list[int], bool, list[ImpatienceEvent]] = ([], False, [ImpatienceEvent("arrival", 0)])):
        # Reset clock
        self._clock = 0
        ...
        while True:
            ...
            if current_event.type == "arrival":
                ...
                self._future_events.append(
                    ImpatienceEvent("arrival", self._interarrival_distro() + self._clock)
                )

                # Create an impatience event
                self._future_events.append(
                    ImpatienceEvent("impatience", self._impatience_distro() + self._clock, _arrival_index)
                )
            ...
            if current_event.type == "impatience":
                if current_event.customer in self._queue:
                    self._queue.remove(current_event.customer)
                    lost_cars += 1
            ...
            if self._serving == False and len(self._queue) > 0:
                ...
                self._future_events.append(
                    ImpatienceEvent("departure", self._service_distro() + self._clock)
                )
    ...
    def simulate(self, days : int):
        ...
        file_name = "impatient_customers_simulation.csv"
        ...
        with open(file_name, mode='w', newline='') as file:
            ...
            for _ in range(0, days):
                writer.writerow(self._simulate_day([[], False, [ImpatienceEvent("arrival", self._interarrival_distro())]]))

def arrival_distro():
    return floor(expovariate(20 / 3600))
def service_distro():
    return floor(expovariate(1 / 720))
def impatience_distro():
    while True:
        # Truncate normal distribution to get only positive values
        value = floor(normalvariate(1800, 1))
        if value > 0:
            return value

car_wash = CarWash(arrival_distro, service_distro, impatience_distro)
